...
const ParentHistory = () => {
    ...
    useEffect(() => {
        const fetchHistory = async () => {
            try {
                const res = await api.get('/parents/child-history'); // Parent endpoint
                setHistory(res.data);
                setLoading(false);
            } catch (error) {
                console.error(error);
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 py-8">
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Child Learning History</h1>
            <p className="text-gray-500 mb-8">Track your child's progress and completed activities covering {totalCompleted} items.</p>
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <h3 className="text-2xl font-bold text-gray-900">
                            {completedVideos.length} <span className="text-sm text-gray-400 font-normal">/ {history.stats?.totalVideos || '-'}</span>
                        </h3>
                    </div>
                </div>
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <h3 className="text-2xl font-bold text-gray-900">
                            {submissions.length} <span className="text-sm text-gray-400 font-normal">/ {history.stats?.totalQuizzes || '-'}</span>
                        </h3>
                    </div>
                </div>
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4">
                ...
                {(activeTab === 'all' || activeTab === 'quizzes') && submissions.map((sub) => (
                    <div key={`sub-${sub._id}`} className="bg-white p-4 rounded-xl border border-gray-100 flex justify-between items-center group hover:border-purple-100 transition-colors">
                        ...
                        <div className="text-right">
                            <span className={`block font-bold ${sub.score >= 50 ? 'text-green-600' : 'text-red-600'}`}>

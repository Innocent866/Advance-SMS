...
const StudentHistory = () => {
    usePageTitle('Learning Analytics Hub');
    ...
    useEffect(() => {
        const fetchHistory = async () => {
            try {
                ...
                console.error('History telemetry sync failed:', error);
                setLoading(false);
            }
        };
        fetchHistory();
                ...
                <div className="relative flex flex-col lg:flex-row justify-between items-center gap-12">
                    <div className="space-y-6 text-center lg:text-left">
                        <div className="inline-flex items-center gap-3 px-4 py-2 bg-white/5 backdrop-blur-3xl rounded-full border border-white/10 text-[10px] font-black uppercase tracking-[0.3em] text-primary">
                            <Clock size={14} /> Temporal Learning Stream
                        </div>
                        <h1 className="text-5xl lg:text-6xl font-black tracking-tight leading-tight text-balance">
                            Learning <br />
                            <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary via-indigo-400 to-purple-400 italic">Analytics Hub</span>
                        </h1>
                        <p className="text-slate-400 font-medium max-w-xl text-lg">
                            An immutable ledger of your academic engagements, pedagogical milestones, and intellectual growth metrics.
                        </p>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-6">
                        <div className="bg-white/5 backdrop-blur-3xl p-8 rounded-[2.5rem] border border-white/10 shadow-2xl min-w-[200px] group transition-all hover:bg-white/10">
                            ...
                            <div className="mt-4 flex items-center gap-2">
                                ...
                                <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">Live Telemetry</span>
                            </div>
                        </div>

                        <div className="bg-white/5 backdrop-blur-3xl p-8 rounded-[2.5rem] border border-white/10 shadow-2xl min-w-[200px] group transition-all hover:bg-white/10">
                            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest leading-none mb-3">Mastery Index</p>
                            ...
                            <div className="mt-4 flex items-center gap-2 text-indigo-400">
                                <Trophy size={14} />
                                <span className="text-[9px] font-black uppercase tracking-[0.2em]">Neural Validated</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
                ...
                >
                    ...
                    <div>
                        ...
                        <h3 className="text-2xl font-black text-slate-900">{completedVideos.length} Lessons</h3>
                    </div>
                </motion.div>
                ...
                >
                    ...
                    <div>
                        ...
                        <h3 className="text-2xl font-black text-slate-900">{submissions.length} Completed</h3>
                    </div>
                </motion.div>
            ...
            <div className="space-y-6">
                            ...
                            <div className="flex items-center gap-6 relative z-10">
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                                        {item.videoId?.subjectId?.name} • Initialized {new Date(item.watchedAt).toLocaleDateString(undefined, { day: 'numeric', month: 'short', year: 'numeric' })}
                                    </p>
                                </div>
                            </div>
                        ...
                        >
                            ...
                            <div className="flex items-center gap-4 relative z-10 w-full md:w-auto justify-end">
                                <div className="bg-slate-50 px-6 py-2 rounded-xl border border-slate-100 flex flex-col items-center">
                                    ...
                                    <span className={`text-xl font-black tracking-tighter ${sub.passed ? 'text-emerald-600' : 'text-rose-600'}`}>
                ...
                {completedVideos.length === 0 && submissions.length === 0 && (
                    ...
                    >
                        ...
                        <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em]">Your instructional trajectory has yet to be archived</p>
                    </motion.div>
                )}
            </div>
        </div>

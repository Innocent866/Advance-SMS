...
const StudentResults = () => {
    ...
    const fetchMetadata = async () => {
        try {
            ...
            showNotification('Failed to load academic sessions', 'error');
        } finally {
            setLoading(false);
        }
    };
    ...
    const fetchResults = async () => {
        try {
            ...
            const resResults = await api.get(`/results/my-results?session=${session}&term=${term}`);
            setResults(Array.isArray(resResults.data) ? resResults.data : resResults.data.data || []);
        } catch (error) {
    ...
    const stats = {
        ...
        topGrade: results.length > 0 ? results.some(r => getDisplayGrade(r) === 'A') : false,
                ...
                <div className="relative flex flex-col lg:flex-row justify-between items-center gap-12">
                    <div className="space-y-6 text-center lg:text-left">
                        <div className="inline-flex items-center gap-3 px-4 py-2 bg-white/5 backdrop-blur-3xl rounded-full border border-white/10 text-[10px] font-black uppercase tracking-[0.3em] text-primary">
                            <TrendingUp size={14} /> My Performance
                        </div>
                        ...
                        <p className="text-slate-400 font-medium max-w-xl text-lg text-balance">
                            Track your academic progress and view your grades across all subjects.
                        </p>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-6">
                        <div className="bg-white/5 backdrop-blur-3xl p-8 rounded-[2.5rem] border border-white/10 shadow-2xl min-w-[220px] group transition-all hover:bg-white/10">
                            ...
                            <div className="mt-4 flex items-center gap-2">
                                ...
                                <span className="text-[9px] font-black text-emerald-400 uppercase tracking-[0.2em]">Latest Data</span>
                            </div>
                        </div>

                        <div className="bg-white/5 backdrop-blur-3xl p-8 rounded-[2.5rem] border border-white/10 shadow-2xl min-w-[220px] group transition-all hover:bg-white/10">
                            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest leading-none mb-3">Distinctions</p>
                            <h3 className="text-5xl font-black text-white leading-none tracking-tighter">{stats.topGrade ? 'Elite' : 'Active'}</h3>
                            <div className="mt-4 flex items-center gap-2 text-indigo-400">
                                <Award size={14} />
                                <span className="text-[9px] font-black uppercase tracking-[0.2em]">Verified Results</span>
                            </div>
                        </div>
                    </div>
                </div>

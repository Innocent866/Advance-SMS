...
const FeaturesSection = () => {
  const items = [
    { role: "School Admin", icon: <Shield className="w-8 h-8"/>, color: "bg-hub-50 text-hub-600", desc: "Gain total control over operations, finance, and staff." },
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                {items.map((item, idx) => (
                    <motion.div 
                      key={idx}
                      className="bg-white p-8 rounded-2xl shadow-sm border border-hub-50 hover:shadow-xl hover:-translate-y-2 transition-all duration-300 group"
